...
const Peminjaman = {
  ...
  searchBarang: (query, callback) => {
    db.query(
      'SELECT b.id_barang, b.nama_barang, l.nama_lab, b.stok, b.gambar ' + // Tambah gambar
      'FROM barang b ' +
      'JOIN lab l ON b.id_lab = l.id_lab ' +
  ...
  getBarangByLab: (id_lab, callback) => {
    db.query(
      'SELECT b.id_barang, b.nama_barang, b.stok, b.gambar, l.nama_lab, COUNT(p.id_peminjaman) as dipinjam ' + // Tambah gambar
      'FROM barang b ' +
      'JOIN lab l ON b.id_lab = l.id_lab ' +
      ...
      'GROUP BY b.id_barang, b.nama_barang, b.stok, b.gambar, l.nama_lab', // Tambah gambar di GROUP BY
      [id_lab],
      (err, results) => {
        if (err) return callback(err);
        callback(null, results);
  ...
  createBarang: (barangData, callback) => {
    const { id_lab, nama_barang, stok, gambar } = barangData; // Tambah gambar
    db.query(
      'INSERT INTO barang (id_lab, nama_barang, stok, gambar) VALUES (?, ?, ?, ?)', // Tambah gambar
  ...
  updateBarang: (id, barangData, callback) => {
    const { nama_barang, stok, gambar } = barangData; // Tambah gambar
    db.query(
      'UPDATE barang SET nama_barang = ?, stok = ?, gambar = ? WHERE id_barang = ? AND is_deleted = 0', // Tambah gambar
