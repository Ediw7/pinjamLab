...
function KelolaBarang() {
  ...
  const handleAddBarang = async (e) => {
    ...
    // Validation
    if (!newBarang.id_lab) {
      showToast('Pilih lab terlebih dahulu', 'error');
      return;
    }
    ...
    if (!newBarang.stok || newBarang.stok < 0) {
    ...
    try {
      ...
      showToast(`Barang berhasil ditambahkan dengan ID: ${res.data.id}`, 'success');
      
      if (newBarang.id_lab === selectedLab) {
        setBarang([
          ...barang,
          { id_barang: res.data.id, nama_barang: newBarang.nama_barang, stok: newBarang.stok, gambar: newBarang.gambar, dipinjam: 0 },
        ]);
      }
      setNewBarang({ id_lab: '', nama_barang: '', stok: '', gambar: '' });
      setShowPreview(false);
    } catch (err) {
      setError('Add barang failed: ' + (err.response?.data?.message || 'Server error'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleEditBarang = async (e) => {
    ...
    if (!editBarang.nama_barang.trim()) {
      showToast('Nama barang tidak boleh kosong', 'error');
      return;
    }
    
    if (editBarang.stok < 0) {
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 pt-24 pb-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            {selectedLab ? (
              ...
              <div className="bg-white rounded-lg shadow-md p-10">
                <div className="text-center">
                  ...
                  <p className="mt-1 text-sm text-gray-500">
                    Silakan pilih lab di panel samping untuk melihat daftar barang
                  </p>
                </div>
              </div>
            )}
            ...
            {selectedLab && filteredBarang.length > 0 && (
              <div className="bg-white rounded-lg shadow-md p-6">
                <h3 className="text-lg font-semibold text-gray-800 mb-4 flex items-center">
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2 text-blue-600" viewBox="0 0 20 20" fill="currentColor">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                  ...
                  <div className="bg-green-50 p-4 rounded-lg border border-green-100">
                    ...
                    <p className="text-2xl font-bold text-green-800">
                      {barang.reduce((total, item) => total + (parseInt(item.stok) || 0), 0)}
                    </p>
                  </div>
                  <div className="bg-yellow-50 p-4 rounded-lg border border-yellow-100">
                    <p className="text-sm text-yellow-600 font-medium">Sedang Dipinjam</p>
                    <p className="text-2xl font-bold text-yellow-800">
                      {barang.reduce((total, item) => total + (parseInt(item.dipinjam) || 0), 0)}
                    </p>
                  </div>
                  <div className="bg-purple-50 p-4 rounded-lg border border-purple-100">
                    <p className="text-sm text-purple-600 font-medium">Tersedia</p>
                    <p className="text-2xl font-bold text-purple-800">
                      {barang.reduce((total, item) => total + (parseInt(item.stok) || 0) - (parseInt(item.dipinjam) || 0), 0)}
                    </p>
                  </div>
                </div>
              </div>
